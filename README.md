...
The separately versioned agentic candidate can be fetched with `.venv/bin/python scripts/download_artifacts.py --agentic-candidate`; its manifest verifies each file. Use `--onnx-only` for the FP32 ONNX runtime without PyTorch weights. This is an opt-in experimental checkpoint with its own tokenizer, calibration and policy files. The default demo and API continue to use the established model unless a model path is explicitly provided.
