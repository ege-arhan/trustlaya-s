...
Adding paired agentic training data produced another isolated candidate. On its scenario-isolated 545-row test, F1 was **0.872** versus v2 **0.628**, with false-positive rate **0.211** versus **0.864**. On separate broad PromptWall data, recall fell **0.723→0.698** while false-positive rate fell **0.231→0.062**. AgentInjectionBench benign false-positive rate remained **0.800**. The candidate is not promoted; these datasets measure different attack and benign distributions and do not establish a production operating point.

Human-written Bordair game submissions provide an attack-intent-only diagnostic, not an F1 benchmark. On 855 recorded bypass strings, detection at each model's configured threshold was 71.3% for v2 and 36.4% for the agentic candidate. The source lacks benign controls and per-string injection adjudication; details and pinned revision are in `reports/live_redteam_diagnostic.json`.

On the untouched 942-row NeurAlchemy grouped test, broad injection/jailbreak F1 was 0.632 for v2 and 0.805 for the agentic candidate; attack recall was 0.498/0.726 and benign FPR 0.110/0.110. Normalized exact overlap with training/development text was zero. Source mixtures and semantic overlap remain possible. Full counts: `reports/neuralchemy_cross_source.json`.
