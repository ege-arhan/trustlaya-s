...
## Versioned fail-closed authorization milestone

The later feature branch adds `/v1/authorize` and `/v1/consume` to the same API, backed by a trusted tool allowlist and a 15-second, single-use opaque token. A guarded tool executes only after consuming a matching authorization for the exact request ID, agent/session, tool, operation, target, permissions and payload hash. REDACT permits only a sanitized payload that passes policy reanalysis; REVIEW and BLOCK issue no token. Model weights and existing policy rules were not changed. The seven-case real-ONNX demo and all 60 tests passed, including replay, payload-mismatch and gateway-shutdown denials. The Mac local gateway benchmark measured total p50/p95/p99 5.568/5.939/6.051 ms with a fake no-op tool; it is not an UNO Q result. Full protocol, test and deployment limitations are in `docs/authorization_protocol.md`, `docs/security_boundary.md`, `docs/uno-q-deployment.md` and `reports/authorization_milestone.md`.
