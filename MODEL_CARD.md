...
On a separate Apache-2.0 agentic tool-output benchmark (142 attacks, 40 matched benign), the injection head achieved F1 **0.484**, recall **0.380**, FPR **0.675** at 0.5. Overlapping-window maximum scoring improved F1 to **0.592** but did not reduce FPR. This measured text detection, not attack execution by an agent. Policy requires REVIEW for declared untrusted tool output entering an unapproved privileged agent, regardless of classifier score; the caller must provide correct metadata and enforce the pause. [Breakdown](reports/agent_injection_independent.json).

Later isolated injection-head experiments improved a new Turkish/English cross-source diagnostic. A paired agentic-data candidate improved its scenario-isolated test F1 from 0.628 to 0.872 and reduced false-positive rate from 0.864 to 0.211, but flagged 80% of benign AgentInjectionBench tool returns and missed attacks on a separate broad corpus. These candidates are **not** this released v2 model. See [experiment decision](reports/injection_experiments.md).

FP32 ONNX is 159.9 MiB and had maximum risk-logit drift 0.000051 versus PyTorch on 256 examples. INT8 ONNX is 40.5 MiB but changed **5.08%** of final policy actions on that sample. Latest saved Mac batch-one warm p50: PyTorch CPU **13.864 ms**, MPS **7.311 ms**, ONNX CPU **7.388 ms**. Measurements are device and run specific. [Benchmark table](benchmarks/results.md).
