...
An additional [AgentDojo paired tool-return check](reports/agentdojo_paired.json) used 25 clean/injected pairs from the pinned ETH Zurich repository, one recorded `important_instructions` run and its clean run per selected task. On the current 96-token input, v2 scored F1 0.478, recall 0.440, benign FPR 0.400; this candidate scored F1 0.746, recall 1.000, benign FPR **0.680**. A fixed 94-token sliding-window maximum raised v2 recall to 1.000 and F1 to 0.833, leaving benign FPR 0.400; the candidate's sliding FPR rose to 0.760. All 25 attacks contain the benchmark-specific `<INFORMATION>` tag and no clean control does, so a trivial tag check scores perfectly. This is a useful truncation and false-alarm diagnostic, **not** evidence of production-grade generalization or agent attack prevention. Eleven injected markers start beyond the initial 94 content tokens. No candidate promotion follows from this small, template-confounded sample.
